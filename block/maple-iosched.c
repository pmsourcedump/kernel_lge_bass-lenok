...
maple_add_request(struct request_queue *q, struct request *rq)
{
	...
	const bool display_idle = is_display_idle();
...
maple_dispatch_requests(struct request_queue *q, int force)
{
	...
	if (!rq) {
		...
		if (display_idle && mdata->starved >= mdata->writes_starved)
			data_dir = WRITE;
		else if (!display_idle && mdata->starved >= 1)
			data_dir = WRITE;
